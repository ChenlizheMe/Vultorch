...
// ======================================================================
//  allocate_shared — Vulkan buffer with external memory, imported to CUDA
// ======================================================================
...
// ======================================================================
//  upload — GPU-GPU copy from arbitrary CUDA ptr
// ======================================================================
...
// ======================================================================
//  sync — staging buffer → VkImage (zero-copy path)
// ======================================================================
void TensorTexture::sync() {
    if (!allocated_) return;

    // The shared buffer is always RGBA (4-channel).  For allocate_shared()
    // the caller must write 4-channel data.  Python create_tensor() and
    // show() guarantee this by padding on GPU before calling.
    if (channels_ != 4)
        throw std::runtime_error(
            "TensorTexture::sync() requires channels == 4 "
            "(Python show()/create_tensor() should pad to RGBA before calling)");

    // Ensure all CUDA writes to the shared buffer are visible to Vulkan
    CU_CHECK(cuStreamSynchronize(nullptr));
...
// ======================================================================
//  allocate_staging_external — VkBuffer with external memory + CUDA import
// ======================================================================
...
void TensorTexture::record_copy(VkCommandBuffer cmd) {
    ...
    // If there's a pending standalone fence (e.g. SceneRenderer triggered a
    // copy earlier this frame), wait for it before reusing the image.
    if (fence_pending_) {
        VK_CHECK(vkWaitForFences(device_, 1, &copy_fence_, VK_TRUE, UINT64_MAX));
        VK_CHECK(vkResetFences(device_, 1, &copy_fence_));
        fence_pending_ = false;
...
void TensorTexture::free_resources() {
    ...
    // CUDA side
    if (cuda_staging_ptr_) { /* cuMemFree not needed — owned by ext mem */ cuda_staging_ptr_ = 0; }
